<?php
/**
 * Holds the (a)squared Login Admin class
 *
...
if (!class_exists('Theme_My_Login_Admin')) :
	...
	class Theme_My_Login_Admin extends Theme_My_Login_Abstract {
		...
		public function admin_menu() {
			add_menu_page(
				__('(a)squared Login Settings', 'theme-my-login'),
				__('(a)squared Login', 'theme-my-login'),
				'manage_options',
				'theme_my_login',
				array('Theme_My_Login_Admin', 'settings_page'),
				ASQUARED_LOGIN_URL . '/admin/assets/img_icon.png'
			);
		...
		public function add_meta_boxes() {
			add_meta_box(
				'tml_action',
				__('(a)squared Login Action', 'theme-my-login'),
		...
		public static function settings_page($args = '') {
			extract(wp_parse_args($args, array(
				'title'       => __('(a)squared Login Settings', 'theme-my-login'),
		...
		public function settings_field_login_type() {
			...
			<ul>
				...
				<li>
					<input name="theme_my_login[login_type]" type="radio" id="theme_my_login_login_type_default" value="default"<?php checked('default', $this->get_option('login_type')); ?> />
					<label for="theme_my_login_login_type_default"><?php _e('Username or E-mail', 'theme-my-login'); ?></label>
				</li>

				<li>
					<input name="theme_my_login[login_type]" type="radio" id="theme_my_login_login_type_username" value="username"<?php checked('username', $this->get_option('login_type')); ?> />
					<label for="theme_my_login_login_type_username"><?php _e('Username only', 'theme-my-login'); ?></label>
				</li>

				<li>
					<input name="theme_my_login[login_type]" type="radio" id="theme_my_login_login_type_email" value="email"<?php checked('email', $this->get_option('login_type')); ?> />
					<label for="theme_my_login_login_type_email"><?php _e('E-mail only', 'theme-my-login'); ?></label>
				</li>
		...
		public function settings_field_modules() {
			foreach (get_plugins(sprintf('/%s/modules', plugin_basename(THEME_MY_LOGIN_PATH))) as $path => $data) {
				...
				<label for="theme_my_login_active_modules_<?php echo $id; ?>"><?php printf(__('Enable %s', 'theme-my-login'), $data['Name']); ?></label>
				<br/>
		...
		public function upgrader_pre_install($response, $args) {
			...
			return new WP_Error('update_denied', sprintf(
				__('(a)squared Login has not been updated because you have not allowed the update on the <a href="%s" target="_top">settings page</a>.', 'theme-my-login'),
